/**
 * Vayves — one-line booking embed for any website or social bio link.
 *
 * <script src="https://vayves.com/embed.js" data-subdomain="your-guesthouse" async></script>
 *
...
(function () {
  ...
  if (!subdomain) {
    console.warn('[vayves] Missing data-subdomain on embed script');
    return;
  }
  ...
  // Default to wherever this script was served from, so an embed keeps working
  // if the platform is rehosted. data-base still wins for deliberate overrides.
  var base = script.getAttribute('data-base') || scriptOrigin(script);
  ...
  window.vayves = window.vayves || {};
  window.vayves.open = openModal;
  window.vayves.close = closeModal;
  // Kept so embeds already published on guesthouse sites keep working.
  window.amaldivesStay = window.vayves;
})();
