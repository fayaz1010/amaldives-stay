...
export function claimVerificationHtml(params: ClaimVerificationParams): string {
  const safeName = escapeHtml(params.guesthouseName);
    ...
    <tr><td align="center">
      <table role="presentation" width="100%" style="max-width:560px;background:#ffffff;border-radius:12px;border:1px solid ${BORDER};overflow:hidden;">
        ...
        <tr>
          <td style="padding:24px 32px;">
            <p style="margin:0 0 16px;font-size:14px;color:${TEXT};line-height:1.55;">
              Confirm you own <strong>${safeName}</strong> to activate your free booking dashboard
              and connect direct bookings from amaldives.com.
            </p>
            <p style="margin:0 0 20px;text-align:center;">
              <a href="${verifyUrl}" style="display:inline-block;background:${TEAL_DARK};color:#ffffff;text-decoration:none;font-weight:600;font-size:15px;padding:12px 24px;border-radius:8px;">
                Verify email &amp; set password
              </a>
            </p>
            <p style="margin:0 0 8px;font-size:12px;color:${MUTED};">Link expires in <strong>${expiresIn}</strong>.</p>
            <p style="margin:0;font-size:12px;color:${MUTED};word-break:break-all;font-family:Menlo,monospace;background:${BG};padding:8px 10px;border-radius:6px;border:1px solid ${BORDER};">
