...
export async function POST(request: NextRequest) {
  try {
    ...
    // The UI sends a Place ID under either `placeId` or the legacy
    // `hotellookId` key — accept both so we don't need to coordinate the
    // client deploy.
    ...
    const photos = Array.isArray(body.photos)
      ? (body.photos as unknown[]).map(String).filter(Boolean).slice(0, 6)
      : [];
      ...
      await tx.tenant.update({
        ...
        data: {
          ...
          settings: {
            ...existingSettings,
            draftMode: true,
            seededFromHotellook: placeId, // legacy key, now holds Place ID
            seededFromPlaceId: placeId,
